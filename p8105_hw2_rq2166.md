...
The number of observation in `Trash_Wheel` is `285`.
...
The number of observation in `precipitation` is `19`. The key variable
in `precipitation` is the amount of precipitation(`total`) in each
month(`month`) in different years(`year`).
...
of years` is from `1947` to `2015`. \#\#\#The key variables are `year`
and `month`.

**Summary of `snp`:**

The variable number in `snp` is `3`. The number of observation in `snp`
is `787`. The `range of years` in `snp` is from `1950` to `2015`.
...
from `1948` to `range(unemployment%>%pull(year))[2]`.

The names of **key variables** in `pols-month`, `snp` and `unemployment`
are `year` and `month`.
