...
The number of observation in `Trash_Wheel` is `285`. And there are `14`
variables in it.
...
The number of observation in `precipitation` is `19`, and there are `3`
variables in it. The key variable in `precipitation` is the amount of
precipitation(`total`) in different time.
...
The variable number is `9`, the number of observation `822`, the `range
of years` is from `1947` to `2015`. The dataset is about the number of
national politicians who are democratic or republican at any given time.
The key variables include the number of governors of different parties
associated with the date(`gov_gop`, `gov_dem`), the number of
representatives of different parties on the associated date(`rep_gop`,
`rep_dem`), the number of senators of different parties on the
associated date(`sen_gop`, `sen_dem`) and indicator of whether the
president was republican or democratic on the associated
date(`president`).
